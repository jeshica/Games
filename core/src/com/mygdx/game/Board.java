...
//		}
	...
	public void generate() {
		...
		wallMap.add(new int[]{0,2});
		wallMap.add(new int[]{1,2});
		wallMap.add(new int[]{2,2});
		wallMap.add(new int[]{3,2});
		wallMap.add(new int[]{7,0});
		wallMap.add(new int[]{7,1});
		wallMap.add(new int[]{7,2});
		wallMap.add(new int[]{7,3});
		...
		do {
			...
			for(int[] wall : wallMap){
				_squares[wall[0]][wall[1]] = new Square(Square.numToType(8));
				_squares[wall[0]][wall[1]].mustFall = true;
				_squares[wall[0]][wall[1]].origY = (int)MathUtils.random(-7, -1);
				_squares[wall[0]][wall[1]].destY = wall[1] - _squares[wall[0]][wall[1]].origY;
			}
		...
		for (int x = 0; x < 8; ++x) {
			...
			for (int y = 7; y >= 0; --y) {
				...
				if (_squares[x][y].equals(Square.Type.sqEmpty)) {
					//hasEmpty=true;
					for (int k = y - 1; k >= 0; --k) {
							if(!_squares[x][k].equals(Square.Type.sqWall)){
				...
				if (_squares[x][y].equals(Square.Type.sqWall)) {
					
					for (int k = y - 1; k >= 0; --k) {
							if(!_squares[x][k].equals(Square.Type.sqWall)){
								if(_squares[x][k].mustFall){
									if(y-k==_squares[x][k].destY){
										_squares[x][k].destY++;
									}
								}
							}
		...
		for (int x = 0; x < 8; ++x) {
			...
			for (int y = 7; y >= 0; --y) {
				if (_squares[x][y].mustFall == true &&
					...
					!_squares[x][y].equals(Square.Type.sqWall)) {
	...
	public void fillSpaces() {
		for(int x = 0; x < 8; ++x){
	        ...
	        for(int y = 0; y < 8; ++y){
	            if(_squares[x][y].equals(Square.Type.sqEmpty)) {
	                _squares[x][y].setType(Square.numToType(MathUtils.random(2, 7)));
...
//	public MultipleMatch newCheck(){
//		//缓存，分类
//		for(int i=0;i<this._width;i++){
//			for(int j=0;j<this._height;j++){
//				switch(this._squares[i][j].getType()){
//					//case Square.Type.class:
//					//case Square.Type:
//						break;
//					default:
//						break;
